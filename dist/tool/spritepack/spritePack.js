...
async function run() {
    ...
    for (const definitionFile of definitionFiles){
        ...
        const createdSprites = await createSpriteSheet(defintionPath, sourceFilePath);
        if (createdSprites) {
            if (createdSprites.length == 0) {
                console.error(`No sprites created for: ${sourceFilePath}`, createdSprites);
            }
...
async function packSprites(sprites) {
    const options = {
        smart: true,
        pot: false,
        square: false,
    ...
    for(let binIndex = 0; binIndex < packer.bins.length; binIndex++){
        ...
        for (const rect of bin.rects){
            ...
            if (!spritePixels) {
                throw new Error(`Unable to get pixels from ${packedSprite.filename}`);
            }
            const definition = packedSprite.definition;
            let frames = definition.frames;
            if (!frames) {
                console.log(`Frames was not defined ${packedSprite.filename}:${packedSprite.spriteName}. Setting it to 1`);
                frames = 1;
            }
            ...
            for(let x = 0; x < rect.width; x++){
                for(let y = 0; y < rect.height; y++){
                    ...
                    try {
                        ...
                        console.log("Pixel data", spritePixels.shape[0], spritePixels.shape[1]);
                        console.log("Error setting pixel", x, y, xInSourceImage, yInSourceImage, pixel);
        ...
        await bitmap.write(path.join(process.cwd(), "public", "asset", binName));
    }
    // Write all sprites to json
    await fs.writeFile(path.join(process.cwd(), "ts", "generated", "sprites.ts"), "export const sprites = " + JSON.stringify(packedSprites, null, 2));
}
/**
 ...
 * @returns
 */ async function createSpriteSheet(definitionPath, spritePath) {
    ...
    for (const spriteDefinitionEntry of Object.entries(spriteDefinitions)){
        ...
        if (definitionMaxX > width) {
            console.error(`Sprite definition (${spriteName}) width overflow`, definitionMaxX, width);
            return null;
        }
        if (definitionMaxY > height) {
            console.error(`Sprite definition (${spriteName}) height overflow`, definitionMaxY, height);
            return null;
        }
        try {
            const packableSprite = await extractSprite(spriteName, spriteDefinition, pixelData);
...
async function extractSprite(spriteName, spriteDefinition, spritePixels) {
    const options = {
        smart: false,
        pot: false,
        square: false,
    ...
    const packer = new MaxRectsPacker(spriteDefinition.w * frames.length, spriteDefinition.h, 0, options);
    ...
    if (packer.bins.length > 1) {
        throw new Error(`Failed to pack ${spriteName}, got ${packer.bins.length} bins`);
    }
    ...
    for (const packedRectangle of bin.rects){
        ...
        for(let x = 0; x < packedRectangle.width; x++){
            for(let y = 0; y < packedRectangle.height; y++){
                ...
                bitmap.setPixel(packedRectangle.x + x, packedRectangle.y + y, pixel);
            }
        }
    }
    const filename = path.join(process.cwd(), "build", "sprites", `${spriteName}.png`);
...
function getSpriteFrames(spriteDefinition) {
    const frames = [];

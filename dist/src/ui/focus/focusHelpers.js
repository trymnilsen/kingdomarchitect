...
import { closestPointOnLine, manhattanDistance } from "../../common/point.js";
export function getFocusableViews(rootView) {
    return rootView.getViews((view)=>view.isFocusable);
}
export function getClosestFocusableView(focusableViews, currentlyFocusedView, direction) {
    const adjacentViews = getPrioritisedViews(focusableViews.filter((view)=>view != currentlyFocusedView), currentlyFocusedView, direction);
    if (adjacentViews.length > 0) {
        const closestView = closestViewByEdge(currentlyFocusedView, adjacentViews);
        return closestView;
    } else {
        return null;
    }
 ...
 */ function getPrioritisedViews(focusableViews, currentlyFocusedView, direction) {
    ...
    const overlappingViews = viewsInDirection.filter((view)=>{
        ...
        const oppositeEdgeOfFocusedView = getDirectionalEdge(currentlyFocusedView, oppositeDirection);
        const corners = [
            oppositeEdge.start,
            oppositeEdge.end
        ];
        return corners.every((corner)=>isPointPastEdge(direction, corner, oppositeEdgeOfFocusedView.start));
    });
    ...
    const completelyPastEdgeViews = viewsInDirection.filter((view)=>{
        ...
        return view.corners.every((corner)=>isPointPastEdge(direction, corner, edge.start));
    });
...
function getDirectionalEdge(view, direction) {
 ...
 * @param edge
 * @returns
 */ function isPointPastEdge(direction, point, edge) {
    switch(direction){
        case Direction.Left:
 ...
 */ function isViewInDirection(view, originView, direction) {
 ...
 * @param fromView
 * @param views
 */ function closestViewByEdge(fromView, views) {
    if (views.length == 0) {
    ...
    for (const edge of viewEdges){
        const edgePoint = closestPointOnLine(edge.start, edge.end, fromViewPoint);

...
export class UIBox extends UIViewGroup {
    ...
    layout(layoutContext, constraints) {
        ...
        for (const child of this.children){
            ...
            if (this.size.width == wrapUiSize) {
                if (childWidthWithPadding > measuredWidth && childWidthWithPadding < constraints.width) {
                    measuredWidth = childWidthWithPadding;
                }
            }
            if (this.size.height == wrapUiSize) {
                if (childHeightWithPadding > measuredHeight && childHeightWithPadding < constraints.height) {
        ...
        for (const child of this.children){
            if (!child.isLayedOut) {
                throw new Error("Child had no measured size, make its layed out");
            }
            const alignedPosition = calculateAlignment(measuredWidth - horizontalPadding, measuredHeight - verticalPadding, this._alignment, child.measuredSize.width, child.measuredSize.height);
    ...
    draw(context) {
        if (this._background && this.isLayedOut) {
            this._background.draw(context, this.screenPosition, this.measuredSize);
        }
        for (const child of this.children){
            child.draw(context);
        }
    ...
    constructor(...args){
        ...
        _define_property(this, "_background", null);

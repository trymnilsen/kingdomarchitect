 ...
 */ export class ChopTreeJob extends MoveToBeforeJob {
    onDraw(renderContext) {
        ...
        const tilePosition = renderContext.camera.tileSpaceToWorldSpace(this.target.tilePosition);
...
class _ChopTreeJob extends Job {
    ...
    onStart() {
        ...
        if (this.target instanceof SelectedTileItem) {
            const tileComponent = this.entity.getAncestorComponent(TilesComponent);
            if (!tileComponent) {
    ...
    update(tick) {
        ...
        const inventoryComponent = entity.getAncestorComponent(InventoryComponent);
        if (!inventoryComponent) {
            throw new Error("No inventory component on ancestor of entity for ChopTreeJob");
        }
    ...
    onDraw(renderContext) {
        const worldSpacePosition = renderContext.camera.tileSpaceToWorldSpace(this.target.tilePosition);
    ...
    constructor(target){
        super();
        _define_property(this, "target", void 0);
        _define_property(this, "treeEntity", void 0);
        _define_property(this, "blinkingAnimation", void 0);
        const validSelection = target instanceof SelectedTileItem || target instanceof SelectedEntityItem;
        if (!validSelection) {
            throw new JobConstraintsError("Invalid selected provided for chopTreeJob");
        }
        this.target = target;

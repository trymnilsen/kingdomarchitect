...
export function placeSettlement(chunk: TileChunk, _chunkEntity: Entity) {
    ...
    if (procValue < 0.5) {
        log.debug("placeSettlement - Proc less than 0.5, skipping", {
            procValue,
        });
        return;
    }
    log.debug("placeSettlement - Adding settlement", { procValue });
...
function _placeOrcSettlement(chunkEntity: Entity, _position: Point) {
